...
<div class="mx-auto flex min-h-full w-full max-w-7xl flex-col space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
            <p class="text-sm font-medium text-indigo-600">Reports</p>
            ...
            <p class="mt-1 text-sm text-gray-500">
                View student-wise fees by class, section, and payment status.
            </p>
        </div>
    ...
    <div class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
        <div class="flex shrink-0 items-center justify-between border-b border-gray-100 px-5 py-4 sm:px-6">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500">
                    {{ $fees->total() }} student fee records
                </p>
            </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="sticky top-0 z-10 bg-gray-50">
                    <tr>
                        ...
                        <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Class</th>
                        <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Section</th>
                        ...
                        <tr class="transition hover:bg-gray-50">
                            <td class="whitespace-nowrap px-5 py-4">
                                ...
                                <div class="mt-0.5 text-xs text-gray-500">
                                    Roll No: {{ $fee->student->roll_number ?? '-' }}
                                </div>
                            </td>
                            ...
                            <td class="whitespace-nowrap px-5 py-4 text-sm text-gray-600">
                                {{ \Carbon\Carbon::create()->month($fee->month)->format('M') }} {{ $fee->year }}
                            </td>
                            ...
                            <td class="whitespace-nowrap px-5 py-4">
                                @if($fee->status === 'paid')
                                    <span class="inline-flex items-center rounded-full bg-green-50 px-2.5 py-1 text-xs font-semibold text-green-700">Paid</span>
                                @elseif($fee->status === 'partial')
                                    <span class="inline-flex items-center rounded-full bg-yellow-50 px-2.5 py-1 text-xs font-semibold text-yellow-700">Partial</span>
                                @else
                                    <span class="inline-flex items-center rounded-full bg-red-50 px-2.5 py-1 text-xs font-semibold text-red-700">Unpaid</span>
                                @endif
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="8" class="px-5 py-12 text-center">
                                <div class="text-sm font-semibold text-gray-900">No fee records found</div>
                                <p class="mt-1 text-sm text-gray-500">Try changing the class, section, or payment status filters.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
