            ...
            >
                ...
                @foreach ($classes as $class)
                    ...
                    <option
                        value="{{ $class->id }}"
                        @selected(
                            (string) ($filters['class'] ?? '') === (string) $class->id
                        )
                    >
                        {{ $class->name }}
                    </option>
            ...
            >
                ...
                @foreach ($sections as $section)
                    ...
                    <option
                        value="{{ $section->id }}"
                        @selected(
                            (string) ($filters['section'] ?? '') === (string) $section->id
                        )
                    >
                        {{ $section->name }}
                    </option>
                ...
                @foreach ($students as $student)
                    ...
                    <tr class="transition hover:bg-gray-50">
                        ...
                        @foreach ($columns as $column)
                            ...
                            @php
                                $value = data_get($student, $column['key']);
                            @endphp
                        ...
                        {{-- Actions --}}
                        <td
                            class="relative whitespace-nowrap px-5 py-4 text-right"
                        >

                            <details class="student-actions relative inline-block">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.addEventListener('click', function (event) {
        ...
        document
            .querySelectorAll('.student-actions[open]')
    ...
    /*
     * Reload only the Student DataTable.
     */
    function loadStudentTable(form) {

        const table =
            document.querySelector('#student-datatable');
            ...
            if (shouldRestoreSearchFocus) {
                ...
                const nextSearchInput =
                    document.querySelector(
                        '[data-student-filter-form] input[name="search"]'
                    );
    ...
    document.addEventListener('input', function (event) {
        ...
        searchTimer = setTimeout(
            () => loadStudentTable(event.target.form),
            350
        );
    ...
    document.addEventListener('change', function (event) {
        ...
        if (
            event.target.matches(
                '[data-student-filter-form] select'
            )
        ) {
